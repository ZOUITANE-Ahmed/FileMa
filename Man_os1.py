...
def rm(File_Name):
    try:
        if os.path.isfile(File_Name):
            ...
            print(f"{Fore.RED}This file does not exist! \nFile was Removed: {File_Name} Non Successfully{Style.RESET_ALL}")
    except Exception as e:
...
def mkdir(Folder_Name):
    try:
        if not os.path.isdir(Folder_Name):
            ...
            print(f"{Fore.RED}This folder already exists! \nFolder was not created: {Folder_Name} Non Successfully in {os.path.abspath(Folder_Name)}{Style.RESET_ALL}")
    except Exception as e:
        print(f"{Fore.RED}An error occurred: {e}{Style.RESET_ALL}")
...
def size(path_Name):
    try:
        ...
        print(f"{Fore.GREEN}Stat size of {path_Name}: {Fore.WHITE}{size.st_size} bytes{Style.RESET_ALL}")
    except FileNotFoundError:
        print(f"{Fore.RED}File {path_Name} does not exist.{Style.RESET_ALL}")
    except Exception as e:
        print(f"{Fore.RED}An error occurred: {e}{Style.RESET_ALL}")
